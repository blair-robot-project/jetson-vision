...
class CornerChecker:
    ...
    def check_corners(self, corners):
        ...
        if np.array_equal(corners[0], corners[1]) or np.array_equal(corners[0], corners[2]) or np.array_equal(
            ...
            return False
        return np.linalg.norm(corners[0] - corners[3]) > self.min_diag and np.linalg.norm(
            corners[1] - corners[2]) > self.min_diag and corners[0][0] > self.minX and corners[2][0] > self.minX and \
